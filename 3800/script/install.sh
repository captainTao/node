...
opkg install kmod-ipt-conntrack kmod-ipt-nat kmod-ipt-nat-extra
...
SIZE=$(uci get dhcp.@dnsmasq[0].cachesize)
if [[ $SIZE != 1000 ]]; then
	uci add_list dhcp.@dnsmasq[0].cachesize=10000
	uci commit dhcp
else
	echo "The dhcp cachesize is $SIZE, No need to set it up again."
fi
